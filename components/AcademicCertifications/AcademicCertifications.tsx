'use client'

import { motion } from 'framer-motion'

export default function AcademicCertifications() {
  return (
    <section 
      id="education" 
      className="flex flex-col md:flex-row w-full min-h-screen bg-black text-white overflow-hidden relative"
    >
      {/* Left Column - Solid Black Box */}
      <div className="w-full md:w-1/2 bg-black p-8 md:p-16 flex items-center">
        <motion.div
          initial={{ opacity: 0, x: -30 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8, ease: [0.4, 0, 0.2, 1] }}
          className="space-y-8 max-w-xl"
        >
          {/* Top Badge Label */}
          <div className="flex items-center gap-3">
            <div className="h-px w-8 bg-gradient-to-r from-yellow-500 to-yellow-300" />
            <span className="text-xs font-semibold tracking-[0.3em] uppercase text-white/90">
              Academic Background
            </span>
          </div>
          
          {/* Main Heading */}
          <div className="space-y-2">
            <h2 className="text-5xl lg:text-6xl font-serif text-white leading-tight">
              Education
            </h2>
            <h3 className="text-5xl lg:text-6xl font-serif italic text-yellow-400 leading-tight">
              & Learning
            </h3>
          </div>
          
          {/* Sub-description */}
          <p className="text-base lg:text-lg text-white/70 font-sans leading-relaxed max-w-md">
            My academic journey and continuous learning in the field of Artificial Intelligence and Computer Science.
          </p>
          
          {/* Vertical Timeline Tracker */}
          <div className="relative space-y-8">
            {/* Vertical Gold Line */}
            <div className="absolute left-[7px] top-2 bottom-2 w-px bg-gradient-to-b from-yellow-500/80 via-yellow-400/60 to-yellow-500/80" />
            
            {/* Timeline Node 1 */}
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: 0.2, ease: [0.4, 0, 0.2, 1] }}
              className="flex items-start gap-6"
            >
              {/* Glowing Node */}
              <div className="relative flex-shrink-0">
                <div className="w-4 h-4 rounded-full bg-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.7)] animate-pulse" />
              </div>
              
              <div className="space-y-2 flex-1">
                <h4 className="text-xl font-bold text-white">
                  BS in Artificial Intelligence
                </h4>
                <p className="text-sm text-yellow-400/90 font-medium">
                  2023 – Present | The University of Faisalabad
                </p>
                <p className="text-sm text-white/60">
                  Specializing in AI, Deep Learning, and Computer Vision.
                </p>
              </div>
            </motion.div>
            
            {/* Timeline Node 2 */}
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: 0.4, ease: [0.4, 0, 0.2, 1] }}
              className="flex items-start gap-6"
            >
              {/* Glowing Node */}
              <div className="relative flex-shrink-0">
                <div className="w-4 h-4 rounded-full bg-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.7)] animate-pulse" />
              </div>
              
              <div className="space-y-2 flex-1">
                <h4 className="text-xl font-bold text-white">
                  Advanced Core Deep Learning Track
                </h4>
                <p className="text-sm text-yellow-400/90 font-medium">
                  2022 – 2023 | DeepLearning.AI
                </p>
                <p className="text-sm text-white/60">
                  Completed advanced modules on deep learning, neural networks, and AI specialization.
                </p>
              </div>
            </motion.div>
          </div>
        </motion.div>
      </div>
      
      {/* Right Column - Image */}
      <div className="hidden md:block w-full md:w-1/2 h-full min-h-screen">
        <img 
          src="/images/Education.png" 
          alt="Education Background" 
          className="w-full h-full object-cover object-center"
        />
      </div>
    </section>
  )
}